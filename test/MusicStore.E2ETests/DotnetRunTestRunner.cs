...

        {
            ...
            using (StartLog(out var loggerFactory, testName))
            {
                ...
                using (var deployer = ApplicationDeployerFactory.Create(deploymentParameters, loggerFactory))
                {
                    ...
                    var layoutPath = Path.Combine(applicationPath, "Views", "Shared", "_Layout.cshtml");
                    var originalContent = File.ReadAllText(layoutPath);
                    try
                    {
                        File.WriteAllText(layoutPath, originalContent + " ", Encoding.UTF8);

                        logger.LogInformation("Verifying runtime compiled page");
                        response = await RetryHelper.RetryRequest(
                            () => httpClient.GetAsync(string.Empty), logger, deploymentResult.HostShutdownToken);

                        await validator.VerifyRuntimeCompiledHomePage(response);
                    }
                    finally
                    {
                        File.WriteAllText(layoutPath, originalContent);
                    }
